# file: dataloaders/FashionGen.py
import numpy as np
...
class DataLoaderFashionGen(Dataloader):
    ...
    def get_phase(self, phase):
        ...
        for g in genders:
            for c in cats:
                key = "{}_{}".format(g, c)
                combined_gen_cat[key] = list(
                    set(np.where(attributes["gender"] == g)[0].tolist())
                    & set(np.where(attributes["category"] != c)[0].tolist())
                )
        ...
        for i in range(n_train_neg):
            print(
                i,
                n_train_neg,
                "time elapsed: {}".format(time.time() - before),
                end="\r",
            )
            r_idx, c_idx = self.get_hard_negative_training_edge(
                lower_adj.shape[0], lower_adj, attributes, combined_gen_cat
            )
    ...
    def get_test_questions(self):
        ...
        assert (
            len(flat_questions) == len(gt)
            and len(q_ids) == len(gt)
            and len(gt) == len(valid)
        )
    ...
    def yield_test_questions_K_edges(
        self, resampled=False, K=1, subset=False, expand_outfit=False
    ):
        """
        ...
        for question in questions:
            ...
            yield (
                question_adj,
                np.array(outfit_ids),
                np.array(choices_ids),
                np.array(gt),
                np.array(valid),
            )

    def get_hard_negative_training_edge(
        self, num_nodes, lower_adj, attributes, combined_gen_cat
    ):
        """
